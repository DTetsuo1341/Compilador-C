...
static int find_terminal_id(const grammar *g, const char *name)
{
	...
	{
		if(g->terminals[i].symbol && strcmp(g->terminals[i].symbol, name) == 0)
			return i;
	}
	return -1;
}
...
static bool add_symbol_to_array(symbol **arr, int *count, const char *text, bool is_terminal)
{
	...
	/* Reasignr espacio en la memoria*/
	symbol *tmp;
	tmp = realloc(*arr, (*count + 1) * sizeof(symbol));
	if(tmp == NULL)
	{
		return 0;
	}
	...
	*arr = tmp;
    ...
    /* Copiar el texto del símbolo */
    int len = strlen(text);

    (*arr)[*count].symbol = malloc(len + 1);
    if ((*arr)[*count].symbol == NULL)
	{
        return 0;
	}

    strcpy((*arr)[*count].symbol, text);

    /* Llenar metadatos */
    (*arr)[*count].symbol_length = len;
...
static bool compute_first_tables(const grammar *g, bool **first_table, bool **nullable, int *epsilon_id)
{
	...
	int noTerminal = g->num_non_terminals;
	int terminal = g->num_terminals;

	/* Buscar los nulos */
	*epsilon_id = -1;
	for (int i = 0; i < terminal; i++)
	{
		if(strcmp(g->terminals[i].symbol, "epsilon") == 0)
		{
			*epsilon_id = i;
			break;
		}
	...
	/* Reservar memoria */
	*first_table = calloc(noTerminal * terminal, sizeof(bool));
	if(!*first_table)
		return 0;
	*nullable = calloc(noTerminal, sizeof(bool));
    if (!*nullable)
    {
        free(*first_table);
        return false;
	...
	while (changed)
	{
		...
		{
			production produccion = g->productions[p];
			int A = produccion.non_terminal_id;
			...
			for(int i = 0; i < produccion.production_length; i++)
			{
				...
				{	
					...
					int index = A * terminal + simId;
					if(!(*first_table)[index])
					{
						(*first_table)[index] = 1;
						changed = 1;
					}
				...
				{
					/* No terminal*/
					int B = simId - terminal;

					for (int term = 0; term < terminal; term++)
					{
						int idxB = B * terminal + term;
						int idxA = A * terminal + term;
						
						if(term != *epsilon_id && (*first_table)[idxB] && !(*first_table)[idxA])
						{
							(*first_table)[idxA] = 1;
							changed = 1;
						}
	...
	int *out_follow_cols)
{
	...
	int nt = g->num_non_terminals;
	int t = g->num_terminals;

	int cols = t + 1; // + '$'
	int dollar_col = t;

	bool *follow = calloc(nt * cols, sizeof(bool));
	if (!follow)
		return false;

	/* start symbol gets $ */
	follow[dollar_col] = true;
		...
		for (int p = 0; p < g->num_productions; p++)
		{
			...
			for (int i = 0; i < prod.production_length; i++)
			{
				...
				if (suffix_nullable)
				{
					...
					{
						if (follow[A * cols + col] && !follow[B * cols + col])
						{
							follow[B * cols + col] = true;
							changed = true;
						}
					}
				}
	...
	symbol **out_first)
{
	...
	for (int term = 0; term < t; term++)
	{
		if (first_table[non_terminal_id * t + term])
		{
			add_symbol_to_array(&arr, &count,
				g->terminals[term].symbol, true);
		}
	}
	...
	symbol **out_follow)
{
	...
	for (int col = 0; col < follow_cols; col++)
	{
		...
		{
			add_symbol_to_array(&arr, &count,
				g->terminals[col].symbol, true);
		}
		else
		{
			add_symbol_to_array(&arr, &count, "$", true);
		}
	}
...
int compute_first_for_non_terminal(const grammar *g, int non_terminal_id, symbol **out_first)
{
	...
	if (!g || non_terminal_id < 0 || non_terminal_id >= g->num_non_terminals)
		return 0;
...
int compute_follow_for_non_terminal(const grammar *g, int non_terminal_id, symbol **out_follow)
{
	...
	if (!compute_follow_table(
			g, first_table, nullable, epsilon_id,
