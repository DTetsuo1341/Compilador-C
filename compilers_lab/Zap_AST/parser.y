...
function_definition:
    FUNCTION ID LPAREN parameter_list RPAREN block {
        $$ = new Node("Function", "fn " + std::string($2));
        if ($4) $$->add_child($4);
        $$->add_child($6);
    }
    ;
...
elsif_list:
    ...
    | elsif_list ELSIF LPAREN expression RPAREN block {
        Node* elsif_node = new Node("Else-If", "ei");
...
void yyerror(const char *s) {
    // s viene como: "syntax error, unexpected ID, expecting '='"
    // lo limpiamos y coloreamos
    std::string msg(s);

    // quitamos el prefijo generico "syntax error, "
...
int main() {
    std::cout << "--- Starting Zap Parser ---" << std::endl;
    if (yyparse() == 0) {
        std::cout << "\n--- Analysis Success. Function Structure (AST): ---" << std::endl;
        if (root) {
            root->print();
